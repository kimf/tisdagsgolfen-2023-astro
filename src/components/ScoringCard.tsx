      ...
      return (
        <div className="p-4 bg-charcoal-gray rounded-lg shadow-retro-lg">
          ...
          <div className="mt-4 p-4 bg-dark-slate-gray rounded border border-coffee-bean">
            ...
            <pre className="text-xs text-whiteout overflow-auto max-h-40">{JSON.stringify(rawSession, null, 2)}</pre>
          </div>
        </div>
      );
    }
    ...
    return (
      ...
      >
        ...
        >
          <thead>
            <tr>
              <th className="p-2 text-left bg-dark-side border border-coffee-bean font-bold text-ulthuan-grey uppercase text-xs tracking-wider">Spelare</th>
              <th className="p-2 text-center bg-dark-side border border-coffee-bean font-bold text-ulthuan-grey uppercase text-xs tracking-wider">Poäng</th>
              <th className="p-2 text-center bg-dark-side border border-coffee-bean font-bold text-ulthuan-grey uppercase text-xs tracking-wider">Total</th>
            </tr>
          </thead>
          <tbody>
            <AnimatePresence>
              {players.map((player: any) => (
                <motion.tr 
                  ...
                  transition={{ duration: 0.2 }}
                >
                  <td className="p-3 border border-coffee-bean font-medium text-salmon-tint">{player.name}</td>
                  <td className="p-3 border border-coffee-bean">
                    <div className="score-controls flex justify-center items-center gap-3">
                      <motion.button
                        ...
                        onClick={() => handleScoreChange(player.id, Math.max(1, player.scores[currentHole] - 1))}
